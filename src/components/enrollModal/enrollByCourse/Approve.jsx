...
const Approve = ({ course }) => {
  ...
  const handleApprove = async (taiKhoan) => {
    try {
      ...
      if (response.data) {
        toast.success(`User ${taiKhoan} enrolled successfully`);
        getListUser(); // Refresh the list to remove the enrolled user
      }
    } catch (error) {
    ...
    <div id="approve">
      <div className="header">
        <Row gutter={24} justify={"space-between"}>
          <Col span={12}>
            <h5>Students awaiting confirmation</h5>
          </Col>
          <Col span={12}>
            <Search
              placeholder="Search..."
